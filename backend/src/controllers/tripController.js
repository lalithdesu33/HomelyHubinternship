//recv user info
//validate info
//send info to ai trip planner
//calculate budget per night
//search mongodb for suitable properties
// send both ai trip plan + matching propertise
    ...
    try{
        ...
        if(!destination || !budget || !days || !people){
            return res.status(400).json({
                status : "fail",
                message:"please fill in destination,bud,days and people"
            })
        }
        ...
        res.status(500).json({
            status:"fail",
            message:"Could not create a trip plan,Try again"
        })
    }
}
